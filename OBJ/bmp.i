...
u8 stdbmp_decode(const u8 *filename) 
{
	...
	res=f_open(f_bmp,(const TCHAR*)filename,0x01);
	if(res==0)
	{ 
				...
				if(color_byte==3)   
				{
					switch (rgb) 
					{
						case 0:				  
							...
							break ;	   
						case 1: 	 
							color+=((u16)bmpbuf[count]<<3)&0X07E0;
							break;	  
						case 2 : 
							color+=((u16)bmpbuf[count]<<8)&0XF800;
							break ;			
					}   
				}else if(color_byte==2)  
				{
					switch(rgb)
					{
						case 0 : 
							if(biCompression==0)
							{
								color=((u16)bmpbuf[count]&0X1F);	 	
								color+=(((u16)bmpbuf[count])&0XE0)<<1; 
							}else		
							{
								color=bmpbuf[count];  			
							}  
						...
						case 1 : 			  			 
							if(biCompression==0)
							{
								color+=(u16)bmpbuf[count]<<9;  
							}else  		
							{
								color+=(u16)bmpbuf[count]<<8;	
							}  									 
							break ;	 
					}		     
			...
			{	     
				...
				if(color_byte==3)  			
				{
					for(j=0;j<rowcnt;j++)	
					{
						...
						{
							color=(*bmpbuf++)>>3;		   		 	
							color+=((u16)(*bmpbuf++)<<3)&0X07E0;	
							color+=(((u16)*bmpbuf++)<<8)&0XF800;	
				...
				}else if(color_byte==2)
				{
					for(j=0;j<rowcnt;j++)
					{
						if(biCompression==0)
						{
							for(i=0;i<rowpix;i++)
							{
								color=((u16)*bmpbuf&0X1F);			
								color+=(((u16)*bmpbuf++)&0XE0)<<1; 	
		 						color+=((u16)*bmpbuf++)<<9;  	    
							    pic_phy.draw_point(x+tx,y+ty,color);
								tx++;
							}
...
{				
	...
	u16 pixcnt;				   	
	u16 bi4width;		       	
	if(width==0||height==0)return 0x29;	
	if((x+width-1)>lcddev.width)return 0x29;		
	if((y+height-1)>lcddev.height)return 0x29;	
	...
	hbmp.bmiHeader.biCompression=3;
	...
	if(mode==1)res=f_open(f_bmp,(const TCHAR*)filename,0x01|0x02);
 	if(mode==0||res==0x04)res=f_open(f_bmp,(const TCHAR*)filename,0x02|0x04);
...
{				
  ...
  hbmp.bmfHeader.bfOffBits=bmpheadsize;
	...
	if(mode==1)res=f_open(f_bmp,(const TCHAR*)filename,0x01|0x02);
 	if(mode==0||res==0x04)res=f_open(f_bmp,(const TCHAR*)filename,0x02|0x10);
	
 	if(res==FR_OK)
	{
		res=f_write(f_bmp,(u8*)&hbmp,bmpheadsize,&bw);
		atk_8266_send_data_neat_large(link,(u8*)&hbmp,bmpheadsize,(u8 *)"OK",5000);
	}	    
	return res;
}
