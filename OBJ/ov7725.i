#line 1 "..\\HARDWARE\\OV7725\\ov7725.c"
...
#line 2 "..\\HARDWARE\\OV7725\\ov7725.c"
...
#line 3 "..\\HARDWARE\\OV7725\\ov7725.c"
#line 1 "..\\HARDWARE\\OV7725\\ov7725config.h"
#line 4 "..\\HARDWARE\\OV7725\\ov7725config.h"
...
#line 163 "..\\HARDWARE\\OV7725\\ov7725config.h"
...
#line 4 "..\\HARDWARE\\OV7725\\ov7725.c"
...
#line 5 "..\\HARDWARE\\OV7725\\ov7725.c"
...
#line 6 "..\\HARDWARE\\OV7725\\ov7725.c"
#line 7 "..\\HARDWARE\\OV7725\\ov7725.c"
...
u8 OV7725_Init(void)
{
	...
	u16 reg=0;
 	...
 	RCC_APB2PeriphClockCmd(((uint32_t)0x00000004)|((uint32_t)0x00000008)|((uint32_t)0x00000010)|((uint32_t)0x00000020)|((uint32_t)0x00000100)|((uint32_t)0x00000001), ENABLE);
	...
	GPIO_SetBits(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0800)),((uint16_t)0x0100));
 	...
 	GPIO_SetBits(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)),((uint16_t)0x0008)|((uint16_t)0x0010));	
	...
	GPIO_SetBits(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x1400)),((uint16_t)0x0040));
	
	GPIO_InitStructure.GPIO_Pin  = ((uint16_t)0x4000)|((uint16_t)0x8000);  
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
 	GPIO_Init(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x2000)), &GPIO_InitStructure);
	GPIO_SetBits(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x2000)),((uint16_t)0x4000)|((uint16_t)0x8000));
...
void OV7725_Color_Saturation(s8 sat)
{
 	if(sat>=-4 && sat<=4)
	{	
		SCCB_WR_Reg(0xA7,(sat+4)<<4); 
		SCCB_WR_Reg(0xA8,(sat+4)<<4);
	}
}
...
void OV7725_Brightness(s8 bright)
{
	...
	SCCB_WR_Reg(0x9B, bright_value);
	SCCB_WR_Reg(0xAB, sign);
}
...
void OV7725_Contrast(s8 contrast)
{
	...
	{
		SCCB_WR_Reg(0x9C,(0x30-(4-contrast)*4));
	}
}
...
void OV7725_Window_Set(u16 width,u16 height,u8 mode)
{
	...
	if(mode)
	{
		sx=(640-width)/2;
		sy=(480-height)/2;
		SCCB_WR_Reg(0x12,0x06);		
		SCCB_WR_Reg(0x17,0x23); 	
		SCCB_WR_Reg(0x18,0xA0); 	
		SCCB_WR_Reg(0x19,0x07); 	
		SCCB_WR_Reg(0x1A,0xF0); 	
		SCCB_WR_Reg(0x32,0x00);
		SCCB_WR_Reg(0x29,0xA0); 
		SCCB_WR_Reg(0x2C,0xF0); 
	}
	else
	{
		sx=(320-width)/2;
		sy=(240-height)/2;
		SCCB_WR_Reg(0x12,0x46);		
		SCCB_WR_Reg(0x17,0x3f); 	
		SCCB_WR_Reg(0x18, 0x50); 	
		SCCB_WR_Reg(0x19, 0x03); 	
		SCCB_WR_Reg(0x1A, 0x78); 	
		SCCB_WR_Reg(0x32,  0x00);
		SCCB_WR_Reg(0x29,0x50);	
		SCCB_WR_Reg(0x2C,0x78); 
	}
	raw=SCCB_RD_Reg(0x17);
	temp=raw+(sx>>2);
	SCCB_WR_Reg(0x17,temp);
	SCCB_WR_Reg(0x18,width>>2);
	
	raw=SCCB_RD_Reg(0x19);
	temp=raw+(sy>>1);
	SCCB_WR_Reg(0x19,temp);
	SCCB_WR_Reg(0x1A,height>>1);
	
	raw=SCCB_RD_Reg(0x32);
	temp=((sy&0x01)<<6)|((sx&0x03)<<4)|((height&0x01)<<2)|(width&0x03)|raw;
	SCCB_WR_Reg(0x32,temp);
	
	SCCB_WR_Reg(0x29,width>>2);
	SCCB_WR_Reg(0x2C,height>>1);
	
	SCCB_RD_Reg(0x2A);	
	temp = (raw|(width&0x03)|((height&0x01)<<2));	
	SCCB_WR_Reg(0x2A,temp);	
}
