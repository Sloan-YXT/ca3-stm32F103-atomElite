...
//串口接收缓存区 	
u8 USART3_RX_BUF[USART3_MAX_RECV_LEN+1]; 				//接收缓冲,最大USART3_MAX_RECV_LEN个字节.
u8  USART3_TX_BUF[USART3_MAX_SEND_LEN+1]; 			//发送缓冲,最大USART3_MAX_SEND_LEN字节
...
void USART3_IRQHandler(void)
{
		...
		//USART3的中断处理中如果执行Printf usaet3 buffer就会乱，因为新来的接受不到了
		//printf("%c:%d\n",res,res);
		if((USART3_RX_STA&~(1<<15|1<<14))<USART3_MAX_RECV_LEN)	//还可以接收数据
		{
				//printf("*****\r\n");
			TIM_SetCounter(TIM7,0);//计数器清空          				//计数器清空
			if((USART3_RX_STA&(1<<15))==0) 				//使能定时器7的中断 
			{
...
{  
  ...
  USART_ITConfig(USART3, USART_IT_RXNE, ENABLE);//开启中断   
	...
	TIM7_Int_Init(50-1,7200-1);		//50ms中断,排除数据收不完的可能性
	USART3_RX_STA=0;		//清零
